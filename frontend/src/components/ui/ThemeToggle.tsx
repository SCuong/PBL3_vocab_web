...
export const ThemeToggle = ({ className = '' }: ThemeToggleProps) => {
    ...
    return (
        ...
        >
            ...
            />
            <span className={`absolute left-1 top-1 z-10 grid h-7 w-7 place-items-center leading-none transition-colors duration-300 ${isDark ? 'text-text-muted/70' : 'text-warning-color'}`}>
                <Sun className="block h-4 w-4" aria-hidden="true" />
            </span>
            <span className={`absolute right-1 top-1 z-10 grid h-7 w-7 place-items-center leading-none transition-colors duration-300 ${isDark ? 'text-[#4c1d95]' : 'text-text-muted/70'}`}>
