...
export const StreakModal = ({ isOpen, onClose, gameData }: StreakModalProps) => {
    ...
    // Prefer backend analytics (same source as dashboard/profile/navbar); fall back to local.
    const streakValue = learnerAnalytics?.streak ?? Number(gameData?.currentUser?.streak ?? 0);
    const historyDates = learnerAnalytics
        ? learnerAnalytics.heatmap
            .filter((day) => day.wordsStudied > 0)
            .map((day) => day.date.slice(0, 10))
        : (Array.isArray(gameData?.currentUser?.studyHistory) ? gameData.currentUser.studyHistory : []);
    const groupStreak = Array.isArray(gameData?.groupStreak) ? gameData.groupStreak : [];
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            >
                ...
                <div className="text-center mb-10">
                    ...
                    <h2 className={`${typography.sectionTitle} mb-2`}>{streakValue} Ngày Streak!</h2>
                ...
                <div className="grid md:grid-cols-2 gap-8 mb-10">
                    <div className="bg-bg-light p-6 rounded-2xl border border-primary/10">
                        ...
                        <StreakHeatmap history={historyDates} />
                    ...
                    <div className="bg-bg-light p-6 rounded-2xl border border-primary/10">
                        ...
                        <div className="space-y-3">
                            {[7, 30, 100].map(m => (
                                <div key={m} className={`flex items-center gap-3 p-2 rounded-lg border ${streakValue >= m ? 'bg-primary/10 border-primary/20' : 'opacity-40 border-transparent'}`}>
                                    <div className="text-xl">{m === 7 ? '🔥' : m === 30 ? '🏆' : '👑'}</div>
                                    <div className="text-sm font-bold">{m} Ngày</div>
                                    {streakValue >= m && <Check size={14} className="ml-auto text-green-600" />}
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="glass-card p-6 border-accent/30 bg-accent/5">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Badge variant="accent">Demo · sắp ra mắt</Badge>
                    </div>

                    <div className="flex gap-4 mb-6">
                        {groupStreak.map((member: any) => (
