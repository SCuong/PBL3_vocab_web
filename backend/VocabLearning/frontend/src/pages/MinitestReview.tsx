...
type MinitestReviewProps = {
  detail?: {
    review?: TranslationReviewItem[];
  };
  onBack: (page: string) => void;
};

const MinitestReview = ({ detail, onBack }: MinitestReviewProps) => {
  const items = Array.isArray(detail?.review) ? detail?.review : [];
    ...
    <div className="max-w-4xl mx-auto px-6 py-24">
      <div className="glass-card p-12 border-2 border-primary/10">
        ...
        <div className="mt-10 flex justify-end">
          <Button variant="primary" className="px-10 py-4" onClick={() => onBack('minitest-result')}>
            Quay lại kết quả
          </Button>
        </div>
      </div>
