...
const Vocabulary = ({ onNavigate, onSelectWord }: any) => {
  const [search, setSearch] = useState('');
  const filtered = mockData.vocabulary.filter(v => v.word.toLowerCase().includes(search.toLowerCase()) || v.meaning.toLowerCase().includes(search.toLowerCase()));
        ...
        {filtered.map(v => (
          <div key={v.id} className="glass-card p-6 cursor-pointer group" onClick={() => onSelectWord(v)}>
            <div className="flex justify-between items-start mb-4">
              ...
              <Button variant="ghost" className="p-2 min-h-0 rounded-full"><Volume2 size={16} /></Button>
            </div>
      ...
      <div className="glass-card p-12 relative overflow-hidden">
        <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
          <div className="text-center md:text-left">
            ...
            <Button variant="primary" className="text-lg px-8"><Volume2 size={24} /> Nghe phát âm</Button>
          </div>
          <div className="flex-1 bg-white/50 p-8 rounded-card border-2 border-primary/10 shadow-sm">
            ...
            <h3 className="text-xl font-bold mb-4 text-purple">Ví dụ</h3>
    ...
    <div className="max-w-md mx-auto px-6 py-24">
      <div className="glass-card p-10">
        ...
        <div className="space-y-4 mb-8">
          <input type="email" placeholder="Email" className="w-full px-4 py-3 rounded-xl border-2 border-primary/10" />
          <input type="password" placeholder="Mật khẩu" className="w-full px-4 py-3 rounded-xl border-2 border-primary/10" />
          <Button variant="primary" className="w-full" onClick={() => onLogin({ username: 'You', email: 'user@example.com', role: 'STUDENT', status: 'Active' })}>
            {isLogin ? 'Vào học ngay' : 'Tạo tài khoản'}
          </Button>
        </div>
        <p className="text-center text-text-secondary text-sm">
          {isLogin ? 'Chưa có tài khoản?' : 'Đã có tài khoản?'} 
          <button className="text-primary font-bold ml-2 underline" onClick={() => setIsLogin(!isLogin)}>{isLogin ? 'Đăng ký' : 'Đăng nhập'}</button>
        </p>
      </div>
    </div>
  );
...
const LearningTopics = ({ onStartStudy, currentUser, onNavigate }: any) => {
  const [expandedCat, setExpandedCat] = useState<string | null>(currentUser ? 'comm' : 'comm');
    ...
    <div className="max-w-6xl mx-auto px-6 py-12 relative">
      ...
      <div className="space-y-6">
        {mockData.categories.map(cat => {
          ...
          const isCategoryLocked = isGuest && cat.id !== 'comm';
              ...
              >
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-text-muted text-sm">{cat.topicsCount} chủ đề từ vựng</p>
                  </div>
                </div>
                ...
                {isOpen && (
                  ...
                  >
                    <div className="p-8 grid md:grid-cols-2 gap-6">
                      {mockData.topics.filter(t => t.catId === cat.id).map((topic, idx) => {
                            ...
                            {!isGuest && (
                              <div className="mb-6">
                                <div className="h-2 w-full bg-primary/10 rounded-full overflow-hidden mb-1">
                                  <div className="h-full bg-linear-to-r from-primary to-accent transition-all duration-1000" style={{ width: `${(topic.stats?.learned || 0) * 10}%` }} />
                                </div>
                                <div className="text-[10px] text-text-muted text-right font-medium">Tiến độ: {(topic.stats?.learned || 0) * 10}%</div>
                              </div>
                            )}
...
const StudySession = ({ topicId, onFinish, onAddXP, onStreakCheck, onAddToast }: any) => {
  ...
  const words = useMemo(() => mockData.vocabulary.filter(v => v.topicId === topicId), [topicId]);
    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-5xl font-display font-extrabold text-text-primary mb-4 capitalize">{topic?.title}</h1>
            <div className="flex gap-2">
        ...
        {tab === 'flashcard' && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} key="flashcard-tab" className="max-w-2xl mx-auto">
            <div className="relative h-[480px] perspective-1000 mb-12">
              <motion.div onClick={() => setIsFlipped(!isFlipped)} key={currentIndex} initial={{ rotateY: 90 }} animate={{ rotateY: isFlipped ? 180 : 0 }} className="w-full h-full relative preserve-3d cursor-pointer">
...
const Minitest = ({ topicId, onFinish }: any) => {
  ...
  const fillQuestions = useMemo(() => {
    return mockData.vocabulary
      ...
      .map(v => ({
        id: v.id,
        q: v.example.replace(new RegExp(v.word, 'gi'), '________'),
        hint: v.word.charAt(0).toLowerCase(),
        a: v.word
      }));
  }, [topicId]);
  ...
  const handleSubmit = () => {
    ...
    let sFill = 0;
    let sReading = 0;
    fillAnswers.forEach((ans, i) => { if (ans.trim().toLowerCase() === fillQuestions[i].a.toLowerCase()) sFill++; });
    readingAnswers.forEach((ans, i) => { if (ans === passage.questions[i].correct) sReading++; });
    
    let bonus = sReading === passage.questions.length ? 50 : 0;
    if (bonus > 0) confetti({ particleCount: 150, spread: 70, origin: { y: 0.6 } });
  ...
  return (
    <div className="max-w-5xl mx-auto pb-24 relative">
      <div className="sticky top-[80px] z-40 bg-white/90 backdrop-blur-md p-6 rounded-3xl border-2 border-primary/20 shadow-2xl flex items-center justify-between gap-12 mb-16">
        <div className="flex-1">
          ...
          <div className="h-4 bg-purple/10 rounded-full overflow-hidden border border-primary/5">
            <motion.div animate={{ width: `${(answeredCount/totalQuestions) * 100}%` }} className="h-full bg-linear-to-r from-cyan to-primary" />
          </div>
        </div>
      </div>
        ...
        <section id="part1">
           ...
           <div className="grid gap-10">
              {fillQuestions.map((q, i) => {
                ...
                return (
                  <div key={i} className={`glass-card p-10 relative overflow-hidden transition-all duration-500 ${isSubmitted ? (isCorrect ? 'border-green-500 bg-green-50/50' : 'border-red-500 bg-red-50/50') : 'bg-white'}`}>
           ...
           {!isSubmitted && !showingPart2 && <div className="mt-12 flex justify-center"><Button variant="accent" className="px-16 py-5 text-xl" onClick={() => setShowingPart2(true)}>Sang bài 2 →</Button></div>}
        </section>
                     ...
                     <div className="grid sm:grid-cols-2 gap-6">
                        {q.options.map((opt, oi) => (
                           <label key={oi} className={`flex items-center gap-6 p-6 border-2 rounded-3xl cursor-pointer transition-all ${readingAnswers[qi] === oi ? 'border-primary bg-primary/5 shadow-inner' : 'border-primary/5 hover:border-primary/20 bg-white'} ${isSubmitted && oi === q.correct ? 'border-green-500 bg-green-100 text-green-900 font-bold scale-[1.02]' : ''} ${isSubmitted && readingAnswers[qi] === oi && oi !== q.correct ? 'border-red-500 bg-red-100 text-red-900' : ''}`}>
                             <input type="radio" checked={readingAnswers[qi] === oi} disabled={isSubmitted} onChange={() => { const n = [...readingAnswers]; n[qi] = oi; setReadingAnswers(n); }} className="w-6 h-6 accent-primary" />
                             <span className="text-xl">{opt}</span>
                           </label>
                        ))}
                     </div>
                     {isSubmitted && <div className="mt-12 pt-10 border-t border-primary/10"><p className="text-lg text-text-secondary italic leading-loose">💡 Giải thích: {q.explanation}</p></div>}
                  </div>
               ))}
            </div>
          </section>
      ...
      <footer className="mt-24 pt-24 border-t border-primary/10 text-center">
         {!isSubmitted ? (
            <Button variant="primary" className="px-24 py-6 text-3xl shadow-2xl" onClick={handleSubmit}>Nộp bài ngay 🚀</Button>
         ) : (
            <div className="flex flex-col items-center gap-8">
               <Badge variant="green" className="py-4 px-12 text-2xl animate-bounce">KẾT QUẢ ĐÃ GHI NHẬN! 🏆</Badge>
               <Button variant="primary" className="px-12 py-5 text-xl" onClick={() => onFinish(0, 0)}>Hoàn thành chủ đề</Button>
            </div>
         )}
      </footer>
    </div>
    ...
    <div className="grid md:grid-cols-3 gap-8">
      ...
      <div className="md:col-span-2 space-y-8">
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Words', value: 42, icon: <BookOpen className="text-cyan" /> },
            { label: 'Streak', value: user.streak, icon: <Flame className="text-orange-500" /> },
            { label: 'XP', value: user.xp, icon: <Award className="text-pink" /> }
          ].map((s, i) => (
...
export default function App() {
  ...
  const [gameData, setGameData] = useState(mockData.gameData);
      ...
      <main className="pb-24">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {currentPage === 'vocabulary' && <Vocabulary onNavigate={setCurrentPage} onSelectWord={(w: any) => { setSelectedWord(w); setCurrentPage('vocab-detail'); }} />}
            {currentPage === 'vocab-detail' && <VocabDetail word={selectedWord} onBack={() => setCurrentPage('vocabulary')} />}
            {currentPage === 'auth' && <Auth onLogin={(u: any) => { setCurrentUser(u); setCurrentPage('home'); }} />}
            {currentPage === 'learning-topics' && <LearningTopics onStartStudy={(id: any) => { setStudyTopicId(id); setCurrentPage('study-session'); }} currentUser={currentUser} gameData={gameData.currentUser} onNavigate={setCurrentPage} />}
            {currentPage === 'study-session' && <StudySession topicId={studyTopicId} onFinish={handleFinishStudy} onAddXP={addXP} onStreakCheck={triggerStreakCheck} onAddToast={addToast} />}
            {currentPage === 'minitest-result' && <MinitestResult score={testResult?.score} total={testResult?.total} onBack={setCurrentPage} />}
            {currentPage === 'profile' && <Profile user={{...currentUser, ...gameData.currentUser}} onLogout={() => { setCurrentUser(null); setCurrentPage('home'); }} gameData={gameData} onFreezeStreak={() => {}} onOpenStreak={() => setShowStreakModal(true)} />}
        ...
        {!currentUser && currentPage === 'home' && (
          <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-2xl bg-white/80 backdrop-blur-xl border-2 border-primary/30 p-6 rounded-card shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="font-bold text-lg">Đăng ký để lưu tiến độ!</h4>
